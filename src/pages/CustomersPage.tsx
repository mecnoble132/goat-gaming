        ...
        <div className="mx-auto w-full max-w-[1600px] p-3 sm:p-4 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 max-w-2xl">
            <div className="rounded-xl border border-border/50 bg-background/40 p-4 shadow-sm">
              <div className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Total Customers</div>
              <div className="text-2xl font-black text-foreground italic">{stats.totalCustomers}</div>
            </div>
            <div className="rounded-xl border border-border/50 bg-background/40 p-4 shadow-sm">
              <div className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Total Points</div>
              <div className="text-2xl font-black text-primary italic">{stats.totalLoyaltyPoints.toLocaleString()}</div>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border/50 bg-muted/20">
                    <th 
                      className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground cursor-pointer hover:bg-muted/30 transition-colors"
                      onClick={() => toggleSort('name')}
                    >
                      <div className="flex items-center gap-2">
                        Customer
                        <ArrowUpDown size={12} className={sortConfig.key === 'name' ? 'text-primary' : 'text-muted-foreground/50'} />
                      </div>
                    </th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">Contact</th>
                    <th 
                      className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground text-center cursor-pointer hover:bg-muted/30 transition-colors"
                      onClick={() => toggleSort('loyalty_points')}
                    >
                      <div className="flex items-center justify-center gap-2">
                        Loyalty
                        <ArrowUpDown size={12} className={sortConfig.key === 'loyalty_points' ? 'text-primary' : 'text-muted-foreground/50'} />
                      </div>
                    </th>
                    <th 
                      className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground cursor-pointer hover:bg-muted/30 transition-colors"
                    ...
                    >
                      <div className="flex items-center gap-2">
                        Joined
                        <ArrowUpDown size={12} className={sortConfig.key === 'joined' ? 'text-primary' : 'text-muted-foreground/50'} />
                      </div>
                    </th>
                      ...
                      <tr key={customer.id} className="group hover:bg-muted/30 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-bold text-foreground italic">{customer.name || 'Unnamed'}</div>
                              <div className="text-[10px] text-muted-foreground uppercase font-mono tracking-tighter">{customer.id}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                              ...
                              <span className="font-mono text-xs">{customer.whatsapp_number}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
